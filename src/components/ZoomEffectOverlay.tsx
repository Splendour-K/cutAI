...
export const ZoomEffectOverlay = memo(function ZoomEffectOverlay({
  isActive,
  scale,
  focalPoint,
  type,
  ...
  return (
    <>
      ...
      <div className={cn(
        ...
        <span>{zoomPercent > 0 ? '+' : ''}{zoomPercent}%</span>
      </div>

      {/* Focal point indicator (subtle) */}
      {type === 'focus-shift' && (
        <div
          className="absolute w-8 h-8 pointer-events-none z-10 animate-pulse"
          style={{
